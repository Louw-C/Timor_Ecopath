...
chla_data <- read_excel("/Users/louwclaassens/Documents/Documents - Louw’s MacBook Air/WorldFish/Ikan Ba Futura_2023/Science and colabs/Ecological Modeling/Timor-Leste South Coast/Timor_South_Ecopath/Timor_Ecopath/Data/Chl a_Timor-Leste_2019-2024_Monthly Averages_North+South.xlsx")
...
summary_by_location <- chla_data %>%
  group_by(zone) %>%
  summarize(
    mean_chla = mean(`Chl a_mg-m³`, na.rm = TRUE),
    median_chla = median(`Chl a_mg-m³`, na.rm = TRUE),
    min_chla = min(`Chl a_mg-m³`, na.rm = TRUE),
    max_chla = max(`Chl a_mg-m³`, na.rm = TRUE),
    sd_chla = sd(`Chl a_mg-m³`, na.rm = TRUE)
  )
...
p1 <- ggplot(chla_data, aes(x = zone, y = `Chl a_mg-m³`, fill = zone)) +
  geom_boxplot() +
...
p2 <- ggplot(chla_data, aes(x = date_clean, y = `Chl a_mg-m³`, color = zone)) +
  geom_line() +
...
p3 <- ggplot(chla_data, aes(x = season, y = `Chl a_mg-m³`, fill = season)) +
  geom_boxplot() +
...
p4 <- ggplot(chla_data, aes(x = season, y = `Chl a_mg-m³`, fill = zone)) +
...
annual_averages <- chla_data %>%
  group_by(year, zone) %>%
  summarize(mean_chla = mean(`Chl a_mg-m³`, na.rm = TRUE))

# Annual patterns
p5 <- ggplot(annual_averages, aes(x = factor(year), y = mean_chla, fill = zone)) +
...
# North coast heatmap
north_data <- filter(monthly_data, zone == "North")
...
# South coast heatmap
south_data <- filter(monthly_data, zone == "South")
...
t_test_result <- t.test(`Chl a_mg-m³` ~ zone, data = chla_data)
print("T-test comparing North and South Coast Chlorophyll a:")
print(t_test_result)

# 5.2 ANOVA for seasonal differences
anova_season <- aov(`Chl a_mg-m³` ~ season, data = chla_data)
...
anova_interaction <- aov(`Chl a_mg-m³` ~ zone * season, data = chla_data)
...
north_data <- filter(chla_data, zone == "North")
south_data <- filter(chla_data, zone == "South")
...
# Linear models
north_trend <- lm(`Chl a_mg-m³` ~ date_numeric, data = north_data)
south_trend <- lm(`Chl a_mg-m³` ~ date_numeric, data = south_data)
...
# Plot the trends
p9 <- ggplot(chla_data, aes(x = date_clean, y = `Chl a_mg-m³`, color = zone)) +
